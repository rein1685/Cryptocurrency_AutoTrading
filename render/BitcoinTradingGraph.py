...
class BitcoinTradingGraph:
    ...
    def render(self, current_step, net_worth, trades, window_size=40):
        ...
        #print(self.df['Timestamp'].values[step_range][0])
        #print(datetime.utcfromtimestamp(self.df['Timestamp'].values[step_range][0] / 1000).strftime('%Y-%m-%d %H:%M'))

        date_labels = np.array([datetime.utcfromtimestamp(x/1000).strftime(
